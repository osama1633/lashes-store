...
CREATE POLICY "Authorized account claims admin role"
...
WITH CHECK (
  ...
  AND lower(COALESCE(auth.jwt() ->> 'email', '')) = ANY (
    ARRAY['abdulazizrashudi@gmail.com'::text,'dody505060607070@gmail.com'::text,'of94086@gmail.com'::text]
  )
);

DROP POLICY IF EXISTS "Authorized admin reads roles" ON public.user_roles;
CREATE POLICY "Authorized admin reads roles"
ON public.user_roles
FOR SELECT
TO authenticated
USING (
