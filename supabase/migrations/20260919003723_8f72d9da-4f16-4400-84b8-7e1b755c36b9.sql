...
BEGIN
  ...
  SELECT * INTO v_shipping_method FROM shipping_methods WHERE id = _shipping_method_id AND is_active FOR SHARE;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    SELECT * INTO v_product FROM products WHERE id = (v_item->>'product_id')::uuid AND is_active FOR UPDATE;
  ...
  IF NULLIF(trim(COALESCE(_coupon_code, '')), '') IS NOT NULL THEN
    SELECT * INTO v_coupon FROM coupons WHERE upper(code) = upper(trim(_coupon_code)) AND is_active AND (starts_at IS NULL OR starts_at <= now()) AND (ends_at IS NULL OR ends_at >= now()) AND minimum_order <= v_subtotal AND (usage_limit IS NULL OR usage_count < usage_limit) FOR UPDATE;
  ...
  INSERT INTO profiles(id, full_name, phone, email, address)
  VALUES(auth.uid(), trim(_customer_name), trim(_customer_phone), _customer_email, _shipping_address)
  ...
  VALUES(_order_number, auth.uid(), trim(_customer_name), trim(_customer_phone), _customer_email, _shipping_address, v_subtotal, v_discount, v_shipping, 0, v_total, v_coupon.id, _payment_code, v_shipping_method.name_ar, _notes)
  ...
  IF v_coupon.id IS NOT NULL THEN UPDATE coupons SET usage_count = usage_count + 1 WHERE id = v_coupon.id; END IF;
...
REVOKE ALL ON FUNCTION public.place_store_order(text,text,text,text,jsonb,uuid,text,text,text,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.place_store_order(text,text,text,text,jsonb,uuid,text,text,text,jsonb) TO authenticated;