DROP TRIGGER IF EXISTS orders_reduce_stock ON public.orders;
...
BEGIN
  ...
  WHERE id = NEW.product_id AND is_active AND stock >= NEW.quantity;
  IF NOT FOUND THEN RAISE EXCEPTION 'Insufficient stock'; END IF;
  RETURN NEW;
END;
$$;
REVOKE ALL ON FUNCTION public.reserve_stock_for_order_item() FROM PUBLIC, anon, authenticated;
CREATE TRIGGER order_items_reserve_stock AFTER INSERT ON public.order_items FOR EACH ROW EXECUTE FUNCTION public.reserve_stock_for_order_item();

CREATE OR REPLACE FUNCTION public.increment_coupon_usage_for_order()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
END;
$$;
REVOKE ALL ON FUNCTION public.increment_coupon_usage_for_order() FROM PUBLIC, anon, authenticated;
CREATE TRIGGER orders_increment_coupon AFTER INSERT ON public.orders FOR EACH ROW EXECUTE FUNCTION public.increment_coupon_usage_for_order();

CREATE POLICY "Customers validate active coupons" ON public.coupons FOR SELECT TO authenticated USING (is_active AND (starts_at IS NULL OR starts_at <= now()) AND (ends_at IS NULL OR ends_at >= now()));
...
BEGIN
  ...
  INSERT INTO profiles(id, full_name, phone, email, address) VALUES(auth.uid(), trim(_customer_name), trim(_customer_phone), _customer_email, _shipping_address)
  ...
  VALUES(_order_number, auth.uid(), trim(_customer_name), trim(_customer_phone), _customer_email, _shipping_address, v_subtotal, v_discount, v_shipping, 0, v_total, v_coupon.id, _payment_code, v_shipping_method.name_ar, _notes) RETURNING id INTO v_order_id;
  FOR v_item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    v_quantity := (v_item->>'quantity')::integer;
